...
print(datumy)
# z datumu udělamé osu x a z pohybu osu y
pohyby_na_uctu = pandas.Series(pohyby, index=datumy)
# dáme do grafu
# pohyby_na_uctu.plot()
# zobrazíme si graf
# plt.show()
# teď chci kumulativní hodnoty
# pohyby_na_uctu.cumsum().plot()
# plt.show()
# můžeme zepšit tak, že přidáme počáteční hodnotu:
# pocatecni_zustatek = 10_000
# pohyby[0] += pocatecni_zustatek
# sloupcový graf:
# pohyby_na_uctu.plot(kind="bar")
# další úpravy grafu:
# pohyby_na_uctu.plot(kind="bar", color= "yellow", grid=True)
# plt.show()
# histogram, kde jsme si sloučili hodnoty po pěti cm a zarovnali k mřížce:
muzi = pandas.Series([
